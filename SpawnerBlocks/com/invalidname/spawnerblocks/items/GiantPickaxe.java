...
public class GiantPickaxe extends Item {
	
	Texture texturebig = null;
	ModelGiantPickaxe ma = null;
	String modeltxtpath = null;

	public GiantPickaxe(String n, String txt, String modeltxt, int uses, int attack, int strength) {
		super(n, txt);
	...
	public void onBlockBroken(Entity ent, int dimension, int x, int y, int z, int wasbid) {
		...
		if(Blocks.getBlock(wasbid).isStone) {
			// Breaks left-right, top-down
			if(blockside == 2 || blockside == 1) {
				for(int i = -1; i < 2; i++) {
					for(int j = -1; j < 2; j++) {
						...
						if(block != null) {
							if(block.isStone && block.mindamage <= stonestrength) {
								//if(i != 0 && j != 0) {
								//	Entity e = ((Player) ent);
								//	InventoryContainer hotbar = e.getHotbar(e.gethotbarindex());
								//	hotbar.currentuses += Math.round(block.maxdamage/stonestrength);
								//	e.setHotbar(e.gethotbarindex(), hotbar);
								//}
